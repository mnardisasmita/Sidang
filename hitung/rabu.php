...
<div class="caption"><center>Jadwal Hari Rabu,
Tanggal
<?php
// konek ke db script
include 'konak.php';
...
$rses = $_SESSION["ruang"] ; 
$querytanggal="SELECT * FROM `jadwal` WHERE `tanggal` = '2016-12-28' AND  `ruang` ='$rses' ORDER BY sesi";
$hasilquerytanggal = mysql_query($querytanggal)or die(mysql_error());
$row = mysql_fetch_array($hasilquerytanggal);
if ($row['tanggal'] == '2016-12-28') 
	{
    echo "28 Desember 2016, ";
	}
else
{
...
<?php
//query yang dibutuhkan
//$querytanggal="SELECT * FROM `jadwalujian2014` WHERE `tanggal` = '2014-12-1' AND  `ruang` =1";
$hasilquerytanggal = mysql_query($querytanggal)or die(mysql_error());
